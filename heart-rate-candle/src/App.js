...
const App = React.createClass({
  ...
  connect() {
    ...
    return this.state.device.gatt.connect().then(server => {
      this.setState({
        server
      });
      return Promise.all([
  ...
  getCandleColor() {
    return this.readCharacteristicValue(CANDLE_COLOR_UUID).then(
      this.decodeString
    );
  },
  setColor(rgb) {
  ...
  handleConnection() {
    ...
    this.request()
      ...
      .then(data => {
        ...
        this.state.device.addEventListener(
  ...
  render() {
    ...
    if (this.state.view === "controls") {
      if (this.state.authenticated && this.state.connected) {
        content = (
          <Candle
            toggleCandle={this.toggleCandle}
